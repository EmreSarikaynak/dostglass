...
module.exports = function(options = {}) {
  ...
  return fetch(`${ baseUrl }/${ archiveName }-${ platform }-${ arch }.zip`).then(archive => {
    ...
    if (offlineErrorCodes.has(error?.code)) {
      console.warn("Rclone indirilirken ağa ulaşılamadı, indirme atlanıyor.");
      console.warn(
        "Mevcut bir rclone kurulumunuz varsa RCLONE_EXECUTABLE ortam değişkeniyle yolunu belirtin ya da bağlantı sağlandığında npm install komutunu tekrar çalıştırın."
      );
      return;
    }
